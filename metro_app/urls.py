from django.urls import path

from metro_app import views

app_name = 'metro'
urlpatterns = [
    # Main page and secondary pages.
    path(r'',
         views.simulation_manager, name='simulation_manager'),
    path(r'howto',
         views.how_to, name='how_to'),
    path(r'tutorial',
         views.tutorial, name='tutorial'),
    path(r'osm_tutorial',
         views.osm_tutorial, name='osm_tutorial'),
    path(r'contributors',
         views.contributors, name='contributors'),
    path(r'disqus',
         views.disqus, name='disqus'),
    # Authentication pages.
    path(r'register',
         views.register, name='register'),
    path(r'register/action',
         views.register_action, name='register_action'),
    path(r'login/error',
         views.login_view, {'login_error': True}, name='login'),
    path(r'login',
         views.login_view, name='login'),
    path(r'login/action',
         views.login_action, name='login_action'),
    path(r'logout',
         views.logout_action, name='logout'),
    path(r'password_reset',
         views.PasswordResetView.as_view(), name='password_reset'),
    path(r'password_reset_done',
         views.PasswordResetDoneView.as_view(), name='password_reset_done'),
    path(r'password_reset_confirm/<uidb64>/<token>/',
         views.PasswordResetConfirmView.as_view(),
         name='password_reset_confirm'),
    # Simulation operation pages.
    path(r'copy',
         views.copy_simulation, name='copy_simulation'),
    path(r'add/action',
         views.simulation_add_action, name='simulation_add_action'),
    path(r'add/import_action',
         views.simulation_import_action, name='simulation_import_action'),
    # Simulation-specific pages.
    path(r'<int:simulation_id>/view',
         views.simulation_view, name='simulation_view'),
    path(r'<int:simulation_id>/export',
         views.simulation_export, name='simulation_export'),
    path(r'<int:simulation_id>/save',
         views.simulation_view_save, name='simulation_view_save'),
    path(r'<int:simulation_id>/edit',
         views.simulation_view_edit, name='simulation_view_edit'),
    path(r'<int:simulation_id>/demand/',
         views.demand_view, name='demand_view'),
    path(r'<int:simulation_id>/demand/add',
         views.usertype_add, name='usertype_add'),
    path(r'<int:simulation_id>/demand/<int:demandsegment_id>/edit/',
         views.usertype_edit, name='usertype_edit'),
    path(r'<int:simulation_id>/demand/<int:demandsegment_id>/edit/save/',
         views.usertype_edit_save, name='usertype_edit_save'),
    path(r'<int:simulation_id>/demand/<int:demandsegment_id>/delete/',
         views.usertype_delete, name='usertype_delete'),
    path(r'<int:simulation_id>/demand/<int:demandsegment_id>/view/',
         views.usertype_view, name='usertype_view'),
    path(r'<int:simulation_id>/demand/import/',
         views.usertype_import, name='usertype_import'),
    path(r'<int:simulation_id>/demand/importzip/',
         views.traveler_import_action, name='traveler_import_action'),
    path(r'<int:simulation_id>/traveler_export',
         views.traveler_simulation_export, name='traveler_simulation_export'),
    path(r'<int:simulation_id>/network/',
         views.network_view, name='network_view'),
    path(r'<int:simulation_id>/run/<int:run_id>/network',
         views.network_view_run, name='network_view_run'),
    path(r'<int:simulation_id>/matrices/<int:demandsegment_id>/',
         views.matrix_main, name='matrix_main'),
    path(r'<int:simulation_id>/matrices/<int:demandsegment_id>/view/',
         views.matrix_view, name='matrix_view'),
    path(r'<int:simulation_id>/matrices/<int:demandsegment_id>/edit/',
         views.matrix_edit, name='matrix_edit'),
    path(r'<int:simulation_id>/matrices/<int:demandsegment_id>/import/',
         views.matrix_import, name='matrix_import'),
    path(r'<int:simulation_id>/matrices/<int:demandsegment_id>/export/',
         views.matrix_export, name='matrix_export'),
    path(r'<int:simulation_id>/matrices/<int:demandsegment_id>/save/',
         views.matrix_save, name='matrix_save'),
    path(r'<int:simulation_id>/matrices/<int:demandsegment_id>/reset/',
         views.matrix_reset, name='matrix_reset'),
    path(r'<int:simulation_id>/pricing/',
         views.pricing_main, name='pricing_main'),
    path(r'<int:simulation_id>/pricing/view/',
         views.pricing_view, name='pricing_view'),
    # path(r'<int:simulation_id>/pricing/edit/',
    # views.pricing_edit, name='pricing_edit'),
    # path(r'<int:simulation_id>/pricing/save/',
    # views.pricing_save, name='pricing_save'),
    path(r'<int:simulation_id>/pricing/export/',
         views.pricing_export, name='pricing_export'),
    path(r'<int:simulation_id>/pricing/import/',
         views.pricing_import, name='pricing_import'),
    path(r'<int:simulation_id>/pricing/reset/',
         views.pricing_reset, name='pricing_reset'),
    path(r'<int:simulation_id>/delete',
         views.simulation_delete, name='simulation_delete'),
    path(r'<int:simulation_id>/run/action',
         views.simulation_run_action, name='simulation_run_action'),
    path(r'<int:simulation_id>/run/<int:run_id>/stop',
         views.simulation_run_stop, name='simulation_run_stop'),
    path(r'<int:simulation_id>/run/<int:run_id>',
         views.simulation_run_view, name='simulation_run_view'),
    path(r'<int:simulation_id>/run_list',
         views.simulation_run_list, name='simulation_run_list'),
    path(r'<int:simulation_id>/run/<int:run_id>/link_output',
         views.simulation_run_link_output, name='simulation_run_link_output'),
    path(r'<int:simulation_id>/run/<int:run_id>/user_output',
         views.simulation_run_user_output, name='simulation_run_user_output'),
    path(r'<int:simulation_id>/run/<int:run_id>/user_path',
         views.simulation_run_user_path, name='simulation_run_user_path'),
    path(r'<int:simulation_id>/public_transit/',
         views.public_transit_view, name='public_transit_view'),
    path(r'<int:simulation_id>/public_transit/list',
         views.public_transit_list, name='public_transit_list'),
    path(r'<int:simulation_id>/public_transit/edit/save',
         views.public_transit_edit_save, name='public_transit_edit_save'),
    path(r'<int:simulation_id>/public_transit/edit',
         views.public_transit_edit, name='public_transit_edit'),
    path(r'<int:simulation_id>/public_transit/delete',
         views.public_transit_delete, name='public_transit_delete'),
    path(r'<int:simulation_id>/public_transit/import',
         views.public_transit_import, name='public_transit_import'),
    path(r'<int:simulation_id>/public_transit/export',
         views.public_transit_export, name='public_transit_export'),
    path(r'<int:simulation_id>/object/<object_name>/',
         views.object_view, name='object_view'),
    path(r'<int:simulation_id>/object/<object_name>/list/',
         views.object_list, name='object_list'),
    # For some reason object_edit_save must be declared before object_edit...
    path(r'<int:simulation_id>/object/<object_name>/edit/save',
         views.object_edit_save, name='object_edit_save'),
    path(r'<int:simulation_id>/object/<object_name>/edit/',
         views.object_edit, name='object_edit'),
    path(r'<int:simulation_id>/object/<object_name>/delete/',
         views.object_delete, name='object_delete'),
    path(r'<int:simulation_id>/object/<object_name>/import/',
         views.object_import, name='object_import'),
    path(r'<int:simulation_id>/object/<object_name>/export/',
         views.object_export, name='object_export'),
    path(r'<int:simulation_id>/demand/<int:demandsegment_id>/export/',
         views.usertype_export, name='usertype_export'),
    # Events pages.
    path(r'events_view', views.show_events, name='events_view'),
    path(r'events_view/add_event', views.create_event, name='events_add'),
    path(r'delete_event/<int:pk>/', views.delete_event, name='events_delete'),
    path(r'events_view/edit_event/show/<int:pk>', views.edit_event_show,
         name='events_edit_show'),
    path(r'events_view/edit_event/edit/<int:pk>', views.edit_event,
         name='events_edit'),
    # Articles pages.
    path(r'articles_view', views.show_articles, name='articles_view'),
    path(r'articles/<path>', views.download_article_file,
         name='article_file_download'),
    path(r'articles_view/add_article', views.create_article,
         name='articles_add'),
    path(r'delete_article/<int:pk>/', views.delete_article,
         name='articles_delete'),
    # Environment pages.
    path(r'environments',
         views.environments_view, name='environments_view'),
    path(r'environments/create',
         views.environment_create, name='environments_create'),
    path(r'environments/add_view/<int:environment_id>',
         views.environment_add_view, name='environments_add_view'),
    path(r'environments/add/<int:environment_id>',
         views.environment_add, name='environments_add'),
    path(r'environments/delete/<int:environment_id>',
         views.environment_delete, name='environments_delete'),
    path(r'environments/delete/<int:environment_id>/<user>',
         views.environment_user_delete, name='environment_user_delete'),
    # Batch pages.
    path(r'<int:simulation_id>/batch/new', views.batch_new, name='batch_new'),
    path(r'<int:simulation_id>/batch/<int:batch_id>/edit',
         views.batch_edit, name='batch_edit'),
    path(r'<int:simulation_id>/batch/<int:batch_id>/delete',
         views.batch_delete, name='batch_delete'),
    path(r'<int:simulation_id>/batch/<int:batch_id>/run/<int:run_id>/cancel',
         views.batch_run_cancel, name='batch_run_cancel'),
    path(r'<int:simulation_id>/batch/<int:batch_id>/save',
         views.batch_save, name='batch_save'),
    path(r'<int:simulation_id>/batch/<int:batch_id>/view',
         views.batch_view, name='batch_view'),
    path(r'<int:simulation_id>/batch_history',
         views.batch_history, name='batch_history'),
]
